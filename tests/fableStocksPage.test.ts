...
describe('Phase 5A — scope held', () => {
  ...
  it('redesigns no page that has not had its own phase', () => {
    ...
    // Phase 5D, and `/macro` in Phase 5F, each migrated to `TableCard` under
    // its own brief — real phase boundaries moving, not a relaxed assertion.
    // They are guarded by `tests/fableWatchlistPage.test.ts` /
    // `tests/fableComparePage.test.ts` / `tests/fableMacroPage.test.ts`, and
    // the pages below still hold the line.
    for (const other of [
      'src/app/page.tsx',
      'src/app/earnings/page.tsx', 'src/app/portfolio/page.tsx',
    ]) {
  ...
  it('leaves the middleware protection lists untouched', () => {
    const mw = read('src/middleware.ts')
    assert.ok(mw.includes('PROTECTED_PAGES'))
    assert.ok(mw.includes('PROTECTED_API'))
    // /stocks appears only in the header comment's list of PUBLIC routes — it
    // must never appear inside either protection array.
    const lists = mw.slice(mw.indexOf('PROTECTED_PAGES'), mw.indexOf('export'))
    assert.ok(!lists.includes("'/stocks'"), '/stocks stays public')
  })
})
