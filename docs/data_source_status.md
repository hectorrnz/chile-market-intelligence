...
| FX table | `fxRates.json` only, no live/persisted path (copper is a separate `macroIndicators.json`/Macro-page indicator, not this table — see below) | `static_mvp` | "Static MVP sample" (fabricated "Source: Bloomberg" removed in 8A) | ✅ Accurate | — | P3: a real FX feed is a new-provider project, not planned |
...
| FX depth table — Chile (`/macro`, CL region) | `fxRates.json` static sample, `CL_FX` id list | `static_mvp` | (unchanged — no badge; static fallback footer) | ✅ Accurate, unchanged this task | — | Untouched by design — Chile FX stays BCCh-only, see §12 of `macro_market_source_coverage.md` |
| FX depth table — US (`/macro`, US region) | **CurrencyFreaks** (unofficial third-party), `GET /api/macro/fx/us`, server-cached 6h | `live` (or `unavailable` if unconfigured/failed) | `SourceStateBadge` ("CurrencyFreaks FX reference (unofficial third-party)") + explicit disclaimer footer + `†` marker on derived (inverted) pairs | ✅ New this task — 12 USD-base pairs (8 direct + 4 inverted); day/YTD change omitted (not fabricated) since the source has no such field | `GET /api/macro/fx/us` | Done — see §12 of `macro_market_source_coverage.md` |
