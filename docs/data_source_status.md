...
| Economic calendar — US (`/macro/calendar`) | FRED Releases API (`FRED_API_KEY`, server-only) via `resolveFredReleaseCalendar()` | `live` (dates-only) | `t.common.datesOnly` + `t.cal.fredSubtitle` ("no consensus, no actual/prior values") | ✅ Accurate — real dates, `actual`/`consensus`/`prior` structurally `null` | `GET /api/macro/fred-release-calendar` | Done |
