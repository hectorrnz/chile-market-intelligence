// Phase 9A/9B — deterministic structured-note term extraction for the two
// original, most-permissive templates: Citi (US style) and HSBC (EU style).
//
// This is also the ROUTER'S FALLBACK PARSER: any document whose issuer isn't
// recognized as Crédit Agricole/BNP Paribas/Barclays/BBVA runs through here
// unchanged from Phase 9B — preserving the exact behavior that already
// extracts 27/45 real term sheets at confidence 1.0. Do not fold new
// issuer-specific label aliases into this file; add a new parser module
// instead (see `creditAgricoleParser.ts` etc.) so this fallback path's
...
export const parseCitiHsbc: IssuerParser = (ctx) => {
  const { lines, joined } = ctx
  const opts = { fileName: ctx.fileName }
  const fields: import('../../types.ts').ExtractedField<unknown>[] = []
  const warnings: string[] = []
  const errors: string[] = []
  const push = (f: import('../../types.ts').ExtractedField<unknown>) => { fields.push(f); return f.value }
  ...
  return { ok: errors.length === 0, parserVersion: CITI_HSBC_PARSER_VERSION, note, fields, warnings, errors, fieldsSeen, fieldsExtracted, fieldsLowConfidence, confidenceScore }
}
