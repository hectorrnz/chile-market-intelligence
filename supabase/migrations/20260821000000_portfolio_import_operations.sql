-- R13.8B Â§ 7/Â§ 8 â€” one import operation owns every mutation a workbook makes.
--
...
-- publication dated at the newest valid frozen reporting date â€” and that a
...
--
--   * `portfolio_import_operations` â€” the durable identity of one import. It
...
--   * `portfolio_import_observation_mutations` â€” a per-identity BEFORE-IMAGE.
...
--   * `portfolio_evolution_observations.import_operation_id` â€” a forward
...
-- publication is never deleted â€” a rollback demotes and re-promotes, and this
...
-- â”€â”€ Guard: the R13.5 publication and R13.R1 history migrations must be applied â”€
do $$
begin
  if to_regclass('public.portfolio_publications') is null then
    raise exception 'public.portfolio_publications is missing â€” apply the R13.5 publication migration first';
  end if;
  if to_regclass('public.portfolio_evolution_observations') is null then
    raise exception 'public.portfolio_evolution_observations is missing â€” apply the R13.R1 history migration first';
  end if;
  if to_regprocedure('public.nmi_publish_portfolio(uuid,date,uuid,text,jsonb,jsonb,text,jsonb)') is null then
    raise exception 'public.nmi_publish_portfolio is missing â€” apply the R13.5 publication migration first';
  end if;
end $$;

-- â”€â”€ 1. The import operation â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
create table if not exists public.portfolio_import_operations (
  ...
  -- The newest valid FROZEN reporting date â€” the one date the current
  ...
  -- NULL means there was none, and a rollback therefore promotes nothing â€”
...
-- â”€â”€ 2. The before-image ledger â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
create table if not exists public.portfolio_import_observation_mutations (
  ...
  -- later rollback of import A â€” which legitimately wrote it first â€” would then
...
-- â”€â”€ 2b. The publication-correction ledger (R13.8D.1) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
--
-- WHY A SECOND LEDGER. Â§ 2 records the before-image of every EVOLUTION identity
...
-- evolution level identical â€” move an amount from net flows into weekly profit
...
-- promotes precisely what it displaced â€” never a heuristic on dates.
--
...
create table if not exists public.portfolio_import_publication_corrections (
  ...
  -- The revision it DISPLACED. Never null in practice â€” a week with no standing
  -- revision is not a restatement, it is a first publication â€” but nullable so
...
-- â”€â”€ 3. Forward lineage on the history rows â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
-- â”€â”€ 4. RLS â€” service-role only â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
--
...
-- â”€â”€ 5. Serialising every import â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
--
...
-- and they are rare â€” weekly, administrator-driven â€” so one transaction-scoped
...
-- â”€â”€ 5b. The material publication comparison (R13.8C.2) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
--
...
-- sociedad total or a performance figure inside the current snapshot â€” that is a
...
-- when BOTH are empty is an import a no-op (Â§ 6).
--
...
--                   and every metadata key except `sourceRow` â€” that metadata
...
-- for performance rows â€” both unique per publication by the R13.5 schema â€” and
...
-- â”€â”€ 6. The import transaction â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
--
...
-- changed is refused, never partially adapted â€” and the before-image written to
...
create or replace function public.nmi_import_portfolio_workbook(
  ...
  -- R13.8D.1 â€” already-published weeks this workbook restates. Each entry is
...
begin
  ...
  -- R13.8D.1 â€” a historical correction may never carry the week being published.
  ...
  -- R13.8C.2 â€” A NO-OP IMPORT IS REFUSED HERE, NOT MERELY DISABLED IN THE
  -- CONSOLE â€” AND A NO-OP IS THE WHOLE IMPORT, NOT ONLY ITS HISTORY.
  --
  ...
  -- append", and the stale published snapshot stayed standing â€” a refusal that
  ...
  -- import produces zero durable financial mutations â€” zero history mutations
  -- AND a current publication materially equivalent to the one already standing
  -- for that week (Â§ 5b defines material, and reads Production's own rows rather
  ...
  -- back â€” which is what makes a snapshot-only correction reversible.
  ...
  --
  -- R13.8D.1 â€” settled history has two forms and both are overwrites: an
  ...
  -- The publication, unchanged. Every refusal it raises â€” blocking findings,
  -- nothing to publish, duplicate submission â€” aborts this whole transaction.
  ...
  -- â”€â”€ R13.8D.1 â€” THE HISTORICAL PUBLICATION CORRECTIONS.
  --
  -- Each already-published week the workbook restates gets the NEXT REVISION OF
  -- ITS OWN DATE, minted by `nmi_publish_portfolio` â€” the same function, with the
  ...
  -- history week â€” which is the whole point of doing this in one RPC instead of
  ...
  loop
    ...
    -- THE DATABASE DECIDES WHAT "RESTATED" MEANS â€” the same Â§ 5b comparison,
...
-- â”€â”€ 7. Reversing an import â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
--
...
-- demoted while the one it displaced â€” if any â€” is promoted back.
--
...
begin
  ...
  -- R13.8D.1 â€” every HISTORICAL week this import corrected must also still be
  ...
  -- Demote before promote, for the partial unique index â€” the same ordering
  ...
  -- R13.8D.1 â€” and the same demote-then-promote for every historical week this
...
-- â”€â”€ 8. Grants â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
-- â”€â”€ 9. Postconditions â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
begin
  ...
  if not exists (
    ...
    raise exception 'portfolio_evolution_observations.import_operation_id is missing â€” rollback could not tell an import''s rows from a later one''s';
  end if;
end $$;
...
begin
  if not exists (
    ...
    raise exception 'the before-image CHECK is missing â€” rollback of a correction could not be exact';
  end if;
  if not exists (
    select 1 from pg_catalog.pg_constraint
    ...
    raise exception 'the (import, identity) uniqueness constraint is missing â€” one import could record two before-images for one identity';
  end if;
  if not exists (
    select 1 from pg_catalog.pg_constraint
    ...
    raise exception 'the correction-reason CHECK is missing â€” an authorized correction could be recorded with no reason';
  end if;
end $$;
...
begin
  foreach tbl in array array[
    ...
    select count(*) into v_pol from pg_catalog.pg_policies
     where schemaname = 'public' and tablename = tbl;
    if v_pol > 0 then
      raise exception '% must have no policy â€” it is service-role only', tbl;
    end if;
...
begin
  ...
  -- R13.8C.2 â€” the import must refuse a packet that mutates nothing, it must
  ...
  -- a future edit from quietly reverting to the R13.8C.1 test â€” counting
  ...
  if src not like '%nmi_portfolio_publication_unchanged%' then
    raise exception 'the no-op guard does not consider the current publication â€” a snapshot-only correction would be refused as nothing to append';
  end if;
